...
def _split_top_level_add_sub(s):
    parts = []
    depth = 0
    start = 0
...
def eval_linear_form(expr, assignment):
    ...
    if re.match(r"^\d+$", s):
        return int(s)
    parts = _split_top_level_add_sub(s)
    if len(parts) <= 1:
...
def _find_matching_paren(s, open_idx):
    """Find matching ')' for '(' at open_idx, handling nesting."""
    depth = 0
...
def _replace_thetas(latex, replacer):
    """Replace all \\theta(...) using balanced-paren matching, not regex."""
    result = ""
    i = 0
    while i < len(latex):
        ...
        result += latex[i:idx]
        open_paren = idx + 6
        close_paren = _find_matching_paren(latex, open_paren)
        if close_paren < 0:
            result += latex[idx:]
            break
...
def substitute_cell(latex, row_assignment, col_assignment):
    ...
    result = _replace_thetas(result, expand_theta_inner)
...
def split_factors(latex):
    ...
    while i < len(s):
        ...
        if s.startswith("\\theta(", i):
            open_paren = i + 6
            close = _find_matching_paren(s, open_paren)
            if close < 0:
